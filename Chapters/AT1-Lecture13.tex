...
Note: we did not prove the theorem, but the key part should be that $|-|:\Set\to\Top$ is a left adjoint, so it preserves all colimits, giving (somehow) that $|Y|/G\cong|Y/G|$.\todo{Not sure about what he was trying to say...} If $G$ happens to be finite, we only need the freeness of the action on $|Y|$ and the fact that $|Y|$ is Hausdorff.
