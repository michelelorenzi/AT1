...
The map $\bar\chi_i$ represents an element in $\pi_n(Y,y)$. Then $\Phi([\incl\circ\chi_i])\in\pi_n(Z,z)$. We choose a representative $\omega_i$ for this class:
...
represents the zero element in $\pi_n(Y,y)$. Since $(*)$ commutes we have $0=\Phi([g])=[f^{(n)}\circ\alpha_j]$, so $f^{(n)}\circ\alpha_j:S^n\to Z$ has a continuous extension to $D^{n+1}$. Hence by the HEP also $f^{(n)}\circ\chi_j|_{S^n}:S^n\to Z$ admits a continuous extension to $D^{n+1}$. Choose such an extension $g_j:D^{n+1}\to Z$ and define
