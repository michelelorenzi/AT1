...
The \textbf{simplicial $n$-simplex} is the represented simplicial set $\Delta[n]:=\Delta(-,[n])$. By the Yoneda lemma, for every simplicial set $X$, the map $$\sSet(\Delta[n],X)\to X_n \qquad (f:\Delta[n]\to X)\mapsto f_n(\id_{[n]})$$
is bijective.

% It is better not to use $Y$ as both simplicial set and space in the same page. Personally I would use $Z$, $W$ to represent simplicial sets.
...
\begin{multline*}
    \Hom_\sSet(Z\times\Delta[1],\S(X))\cong \Mor(|Z\times\Delta[1]|,X)\\
