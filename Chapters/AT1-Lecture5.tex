...
We want to prove the following theorem that we will use in the proof of Hurewicz theorem.
...
\begin{center}
    ...
    (\de\ns\times e_1) \arrow[r, "\bigcup_{i=0,\dots,n} \psi_{d_i^*(f)}{(-,e_1)}"] \arrow[d, hook] & A \arrow[d, hook]\\
...
Step 1.\rightnote{I would rephrase this as follows (see \cite[\nopp III.7.1]{maclane:71}): every presheaf on\\ a small category can be represented as a colimit of representable presheaves (in a canonical way).} Given $H:Z\times\Delta[1]\to Y$ and $z\in Z_n$, we define $\psi^H_z:\Delta[n]\times\Delta[1]\to Y$ by
