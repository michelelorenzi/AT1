...
\begin{tikzcd}
    ...
    & \big(F-s_0(X)\big)/\R_{>0}\to X & F\to X \ar[l,mapsto]
\end{tikzcd}
\]
...
We want to see that the fundamental class $\iota\in H^n(K(A,n);A)$ gives rise to a natural isomorphism:
$$
\ev_{\iota}: [Y,K(A,n)]\longrightarrow H^n(Y;A)\qquad
[f:Y\to K(A,n)]\longmapsto f^*(\iota).
$$
%In the version of date January 8th, you forget to change $f:X\to K(A,n)$ to $f:Y\to K(A,n)$

The convention for $n=0$ is that $K(A,0)$ is just the group $A$ with the discrete topology, the map $\mu:K(A,0)^2\to K(A,0)$ is the addition on $A$ and $i: K(A,0)\to K(A,0)$ is the inverse map, while $\iota\in \Hom(A,A)$ is represented by the identity cocycle.

\begin{theorem}
For all $n\ge0$ and all abelian groups $A$, the evaluation map $\ev_{\iota}$ is a group morphism.
\end{theorem}
...
By Theorem \ref{theorem:correspondence-maps-and-group-morphisms}\todo[color=red]{Does the theorem not require $Y$ to be connected?}, $f$ and $g$ are then based homotopic.

%Answer to "todo": When $Y$ is not connected, suppose $Y=Y_1 \sqcup Y_2$ where $Y_1$ is the connected component of $y$. Then
%\[[Y,K(A,n)]_* \cong [Y_1,K(A,n)]_* \times [Y_2,K(A,n)] \cong [Y_1,K(A,n)] \times [Y_2,K(A,n)] \cong [Y,K(A,n)]\]
...
\textbf{Special case.} Suppose $Y$ is $(n-1)$-connected. Then $H_\ni(Y;\Z)$ is trivial (if $n\ge2$, by Hurewicz) or at least free (if $n=1$), hence $\Ext^1(H_\ni(Y;\Z),\Z)$ vanishes and the morphism $\Phi:H^n(Y;A)\to\Hom(H_n(Y;\Z),A)$ from the UCT is an isomorphism. We also have that for $n\ge2$ the Hurewicz map $h:\pi_n(Y,y)\to H_n(Y;\Z)$ is an isomorphism, while for $n=1$ it is the universal homomorphism into the abelianization. In both cases precomposition yields an isomorphism
\[\Hom(h,A):\Hom_\Grp(H_n(Y;\Z),A)\xto{\cong}\Hom_\Grp(\pi_n(Y,y),A).\]
The composite
\[[Y,K(A,n)]_*\xto{\ev_{\iota}} H^n(Y;A)\underset{\cong}{\xto{\Phi}}\Hom(H_n(Y;\Z),A)\underset{\cong}{\xto{\Hom(h,A)}}\Hom_\Grp(\pi_n(Y,y),A)\]
sends $[f]$ to $\pi_n(f):\pi_n(Y,y)\to\pi_n(K(A,n),*)\to A$, hence we know that it is a bijection by Theorem \ref{theorem:correspondence-maps-and-group-morphisms} (since $Y$ is a $(n-1)$-connected CW-complex and $K(A,n)$ has vanishing homotopy groups for $k>n$). Since the composite, the second and the third maps are bijections, the valuation map $\ev_{\iota}$ must also be.
\end{proof}
